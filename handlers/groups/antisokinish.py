...
sokinishlar2 = ['nx','gey','xuyyet', 'xuet','kt','kot', 'kut','dlb','om','am', 'jlb', 'gone done', 'joul up', ' one nine sky','dnx', 'pnx','sky','кт','кот', 'кут','длб','ом','ам', 'жлб', 'гоне доне', 'жоул уп', ' оне нине скй','днх', 'пнх','скй']
    ...
    async def sokinish(message: types.Message):
        ...
        try:
            ...
            sokinmang_message = await message.answer(f"❗️<b>{message.from_user.mention} vaqtincha mute holatiga tushdingiz!\n</b>"
                                                f"<b>Sabab: <i>Qo'pol so'zlar ishlatgani uchun</i></b>")
            await asyncio.sleep(15)
            await sokinmang_message.delete()
        except aiogram.utils.exceptions.BadRequest as err:
            sokinmang_message = await message.answer(f"❗️<b>Hurmatli {message.from_user.mention} tilingizga qat'iy ehtiyot bo'ling!</b>")
